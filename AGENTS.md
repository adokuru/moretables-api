...
- Always update the api docs both postman and scramble
...
- Reporting review fixes: guest-export list/CSV enable `include_frequency_period`; `filtersMetadata()` includes shift times for the reporting-authorized selector. `percentageChange()` returns null for zero baselines. Turn-time party rows average `settingMinutes` from the same completed samples as the headline; empty headline averages display `—`. `fillCalendarSeries()` supplies chronological zero buckets for covers, source, reservation/walk-in and first/repeat time series, respecting local weekdays; shift category charts retain their categories. Regression coverage lives in `ReportingAccuracyTest.php`.
