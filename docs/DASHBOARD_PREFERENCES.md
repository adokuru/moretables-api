...
Four booleans today. One toggle on the Preferences page — "Always display server sections on floor plan" — is still local-only mock UI, not backed by this endpoint, since it wasn't asked for yet.
...
| `show_cleaned_tables` | boolean | `false` | When `true`, a Finished-bucket guest whose table has already been marked Cleaned stays visible in the Finished section (read-only, no swipe/quick actions, distinct "Cleaned" color) instead of being hidden once resolved. Defaults `false` since hiding a cleaned table is the existing behavior. |
...
- `show_cleaned_tables` (default `false`) — `2026_08_09_032814_add_show_cleaned_tables_to_restaurants_table`.
...
2. It's currently in **`cleaning`** status (`live_status === "cleaning"`, set server-side when a reservation completes). Cleaning tables always stay visible regardless of the filter — staff still need to see them to resolve them via the Finished section's own "Cleaned" quick action (see the `show_cleaned_tables` section below; the floor plan itself is no longer clickable to resolve a cleaning table, that flow moved into the Finished list) — once resolved, if the table isn't otherwise assigned, it drops out of view on the next refetch.

This computation is independent of the `occupiedTables` map used for floor-plan tile *coloring*, which now also tints Finished-bucket tables (bussing_needed/finished), not just Seated ones — see the `show_cleaned_tables` section below — a plain booked-but-not-yet-arrived reservation's table still stays *visible* under this filter without being tinted.
...
## Frontend: `show_cleaned_tables`

A Finished-bucket reservation never actually leaves that bucket (`reservation.status` stays `Completed` forever, `service_stage` just toggles between `bussing_needed`/`finished`) — the only thing that changes once staff confirm the Finished section's "Cleaned" quick action is the reservation's own **table**'s `status` (`RestaurantTableResource.status`, already present on every `FohReservation.table` payload, no new backend field needed for this preference). By default (`show_cleaned_tables: false`), the 3 board components (`AdminFrontLeft.tsx`, `list-view/page.tsx`, `HomeLeft.tsx`) filter out any Finished guest whose `table.status === "available"` before rendering the list — once cleaned, there's nothing left for staff to do, so it disappears.

When `true`, that filter is skipped — a cleaned guest stays visible in the Finished section, but read-only: `FinishedGuestRow`/`FinishedDetailPanel` render it with **no swipe/quick actions at all** (same "zero actions, just listed" pattern the Removed column already uses) and a distinct **"Cleaned"** color/icon (`getGuestStatusConfig("cleaned")`, reuses the icon/orange color freed up when "cleared" was removed from the Seated flow — see `moretable-web-app/AGENTS.md`'s Finished-section redesign notes) instead of the normal bussing_needed/finished tint.
